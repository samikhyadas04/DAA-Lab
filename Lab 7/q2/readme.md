...
Determines the minimum number of drops that **guarantees** finding the
highest floor of an `F`-storey building from which an egg can be dropped
without breaking, given `E` identical eggs.

Classic instance: **2 eggs, 100 floors → 14 drops** (worst case, guaranteed).
